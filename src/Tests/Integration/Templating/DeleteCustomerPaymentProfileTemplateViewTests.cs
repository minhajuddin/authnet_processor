...
namespace Tests.Integration.Templating {
    [TestFixture]
    public class DeleteCustomerPaymentProfileTemplateViewTests {
        ...
        [Test]
        public void RenderDeleteCustomerPaymentProfileTemplateView() {
            var factory = TestHelper.TemplateFactory;
            var template = factory.GetInstance("deleteCustomerPaymentProfileRequest.spark");
            template.Authentication = ObjectMother.TestAuthentication;
            var result = template.Render(_getPaymentProfileAttributes);

            var expectedXml = @"<?xml version=""1.0"" encoding=""utf-8"" ?>
<deleteCustomerPaymentProfileRequest xmlns=""AnetApi/xml/v1/schema/AnetApiSchema.xsd"">  <merchantAuthentication>
    <name>54PB5egZ</name>
